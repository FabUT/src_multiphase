...
!!!---temps
real(PR) :: time_1, time_2
real(PR) :: t_init_Finger=0.0_PR
real(PR) :: t_relax_p_sge_1=0.0_PR
...
subroutine solve_euler(M,dt,cfl) !!!!(M,dt,cfl)
   ...
   t_c2p=0.0_PR ; t_src=0.0_PR ; t_nc2p=0.0_PR ; t_relax_p_sge_1=0.0_PR
   ...
   t_tot=t_c2p+t_src+t_relax_p_sge_1+t_nc2p+t_reset

         
   !print*, 'c2p:',t_c2p/t_tot, 'src:', t_src/t_tot, 'nc2p:', t_nc2p/t_tot, &
   !             'relax:',t_relax_p_sge_1/t_tot,'reset:',t_reset/t_tot
...
subroutine conservative2primitive(U,G,MF) !!!!fl,Yl,rhl,pl,elh,al,bl,cl,rh,vx,vy,vz,p,e,c)
...
subroutine nonconservative2primitive(G,MF) !!!!fl,Yl,rhl,pl,elh,al,bl,cl,rh,vx,vy,vz,p,e,c)
...
real(PR) function pressure_mixt(MF)
   ...
   pressure_mixt=MF%rh*MF%u
   denom=0.0_PR

   do iph=1,Nl
     C1=MF%F(iph)%f/(MF%F(iph)%g_sge-1.0_PR)
     pressure_mixt=pressure_mixt-C1*MF%F(iph)%g_sge*MF%F(iph)%p_sge
     denom=denom+C1
   enddo

   pressure_mixt=pressure_mixt/denom

   if(pressure_mixt.le.0)then

     print*, '********************************************'
     print*, 'PROBLEM PRESSURE MIXT <0'
     if(Nl.ge.2)then
...
real(PR) function energie_elastique(fl, mu,rho0)
   ...
   if(detFT.ne.0.0_PR.and.mu.gt.0.0_PR)then
...
subroutine relax_p_sge_1(MF)
    ...
    t_relax_p_sge_1=t_relax_p_sge_1+t2-t1
...
subroutine Reco_muscl2(Wim1,Wi,Wip1,WL,WR)
   ...
   if(Gradm1.eq.0.0_PR.and.Gradp1.eq.0.0_PR)then

      WL=Wi
      WR=Wi

   elseif(Gradp1.eq.0.0_PR)then
   ...
   integer :: i, j, k, iph
      ...
      enddo 
       ...
       do i=1,N1D
          ...
          !!!---alpha       
          do iph=1,Nl
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%f-MFrec_R(i)%F(iph)%f)
             MFrec_L(i)%F(iph)%f=MFrec_L(i)%F(iph)%f+0.5_PR*dt*SVL(i)*AdW
             MFrec_R(i)%F(iph)%f=MFrec_R(i)%F(iph)%f+0.5_PR*dt*SVR(i)*AdW
          enddo

          !!!---rho     
          do iph=1,Nl
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%rh-MFrec_R(i)%F(iph)%rh)+&
          ...
          !!!---vx  
           ...
           AdW=0.0_PR 
           do iph=1,Nl
               AdW=AdW+MF1D(i)%F(iph)%p*(MFrec_L(i)%F(iph)%f-MFrec_R(i)%F(iph)%f)
               AdW=AdW+MF1D(i)%F(iph)%f*(MFrec_L(i)%F(iph)%p-MFrec_R(i)%F(iph)%p)   
           enddo
           ...
           do iph=1,Nl
              ...
              !!! dep de vx à al
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*dS11dai(k)*(MFrec_L(i)%F(iph)%a(k)-MFrec_R(i)%F(iph)%a(k))
              enddo
              !!! dep de vx à bl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*dS11dbi(k)*(MFrec_L(i)%F(iph)%b(k)-MFrec_R(i)%F(iph)%b(k))
              enddo
              !!! dep de vx à cl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*dS11dci(k)*(MFrec_L(i)%F(iph)%c(k)-MFrec_R(i)%F(iph)%c(k))
              enddo

              AdW=AdW-(MF1D(i)%F(iph)%sig(1,1)+MF1D(i)%p)*(MFrec_L(i)%F(iph)%f-MFrec_R(i)%F(iph)%f)

           enddo

           AdW=AdW/MF1D(i)%rh
           AdW=AdW+MF1D(i)%vx*(MFrec_L(i)%vx-MFrec_R(i)%vx)
          ...
          !!!! dependance de vy aux vecteurs de base
           do iph=1,Nl
              ...
              !!! dep de vy à al
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds12dai(k)*(MFrec_L(i)%F(iph)%a(k)-MFrec_R(i)%F(iph)%a(k))
              enddo
              !!! dep de vy à bl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds12dbi(k)*(MFrec_L(i)%F(iph)%b(k)-MFrec_R(i)%F(iph)%b(k))
              enddo
              !!! dep de vy à cl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds12dci(k)*(MFrec_L(i)%F(iph)%c(k)-MFrec_R(i)%F(iph)%c(k))
              enddo

              AdW=AdW-(MF1D(i)%F(iph)%sig(1,2))*(MFrec_L(i)%F(iph)%f-MFrec_R(i)%F(iph)%f)

           enddo

           AdW=AdW/MF1D(i)%rh
           AdW=AdW+MF1D(i)%vx*(MFrec_L(i)%vy-MFrec_R(i)%vy)
          ...
          !!!! dependance de vz aux vecteurs de base
           do iph=1,Nl
              ...
              !!! dep de vz à al
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds13dai(k)*(MFrec_L(i)%F(iph)%a(k)-MFrec_R(i)%F(iph)%a(k))
              enddo
              !!! dep de vz à bl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds13dbi(k)*(MFrec_L(i)%F(iph)%b(k)-MFrec_R(i)%F(iph)%b(k))
              enddo
              !!! dep de vz à cl
              do k=1,3
               AdW=AdW-MF1D(i)%F(iph)%f*ds13dci(k)*(MFrec_L(i)%F(iph)%c(k)-MFrec_R(i)%F(iph)%c(k))
              enddo

              AdW=AdW-(MF1D(i)%F(iph)%sig(1,3))*(MFrec_L(i)%F(iph)%f-MFrec_R(i)%F(iph)%f)

           enddo

           AdW=AdW/MF1D(i)%rh
           AdW=AdW+MF1D(i)%vx*(MFrec_L(i)%vz-MFrec_R(i)%vz)
          ...
          do iph=1,Nl
             ...
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%p-MFrec_R(i)%F(iph)%p)+&
          ...
          do iph=1,Nl
            do k=1,3
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%a(k)-MFrec_R(i)%F(iph)%a(k))&
          ...
          do iph=1,Nl
            do k=1,3
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%b(k)-MFrec_R(i)%F(iph)%b(k))
          ...
          do iph=1,Nl
            do k=1,3
             AdW=MF1D(i)%vx*(MFrec_L(i)%F(iph)%c(k)-MFrec_R(i)%F(iph)%c(k))
       ...
       do j=1,N1D ; do i=1,Nl+4
          U(i,j)=U(i,j)+dt*dUdt(i,j)
       enddo ; enddo

       do k=1,N1D ; do j=1,11 ; do i=1,Nl
          G(i,j,k)=G(i,j,k)+dt*dGdt(i,j,k)
       enddo ; enddo ; enddo
...
subroutine RK1(dt) !!!(Nl,N,SVL,SVR,ir,F,dt) !!!U,G,dt)
   ...
   real(PR) :: t1,t2,t3,t4,t5,t6
      ...
      dGdt=0.0_PR
       ...
       !if(check_det) call check_rhodet('coucou0')
...
!       print*,  'c2p:', t2-t1, 'src:', t3-t2, 'nc2p:', t4-t3, 'relax:', t5-t4,'reset:', t6-t5
...
subroutine RK2(dt) !!!(Nl,N,SVL,SVR,ir,F,dt)
   ...
   integer :: i, j, k
      ...
      do j=1,N1D ; do i=1,Nl+4
         U(i,j)=U0(i,j)+0.5_PR*dt*dUdt(i,j)
      enddo ; enddo
      do k=1,N1D ; do j=1,11 ; do i=1,Nl
         G(i,j,k)=G0(i,j,k)+0.5_PR*dt*dGdt(i,j,k)
      enddo ; enddo ; enddo
      ...
      do j=1,N1D ; do i=1,Nl+4
         U(i,j)=U0(i,j)+dt*dUdt(i,j)
      enddo ; enddo
      do k=1,N1D ; do j=1,11 ; do i=1,Nl
         G(i,j,k)=G0(i,j,k)+dt*dGdt(i,j,k)
      enddo ; enddo ; enddo
...
subroutine RK2_SSP(dt) !!!(Nl,N,SVL,SVR,ir,F,dt)
   ...
   integer :: i, j, k
      ...
      do j=1,N1D ; do i=1,Nl+4
         U(i,j)=0.5_PR*(U0(i,j)+U(i,j)+dt*dUdt(i,j))
      enddo ; enddo
      do k=1,N1D ; do j=1,11 ; do i=1,Nl
         G(i,j,k)=0.5_PR*(G0(i,j,k)+G(i,j,k)+dt*dGdt(i,j,k))
      enddo ; enddo ; enddo
...
subroutine RK3(dt)   !!!(Nl,N,SVL,SVR,ir,F) !!!,U,G,dt)
   ...
   integer :: i, j, k
      ...
      do k=1,N1D ; do j=1,Nl ; do i=1,Nl
         G(i,j,k)=G0(i,j,k)+dt*dGdt(i,j,k)
      enddo ; enddo ; enddo
      ...
      U1=U ; G1=G !!! etat intermédiaire 1

      if(Reconstruct)then
         call MUSCL !!!!(N1D,F,U,G,FL,FR,UL,UR,GL,GR)
      endif 

      call SRC

      do j=1,N1D ; do i=1,Nl+4
         U(i,j)=0.75_PR*U0(i,j)+0.25_PR*(U1(i,j)+dt*dUdt(i,j))
      enddo ; enddo
      do k=1,N1D ; do j=1,Nl ; do i=1,Nl
         G(i,j,k)=0.75_PR*G0(i,j,k)+0.25_PR*(G1(i,j,k)+dt*dGdt(i,j,k))
      enddo ; enddo ; enddo
      ...
      call relaxation !!!(Nl,N,F,U2,G2)
 
     do i=1,N1D
        call primitive2conservative(MF1D(i),U(1:Nl+4,i),G(1:Nl,1:11,i))
      enddo 
     
      U1=U ; G1=G !!! etat intermediaire 1 : MAJ
      ...
      do j=1,N1D ; do i=1,Nl+4
         U(i,j)=1.0_PR/3.0_PR*(U0(i,j)+2.0_PR*(U1(i,j)+dt*dUdt(i,j)))
      enddo ; enddo
      do k=1,N1D ; do j=1,Nl ; do i=1,Nl
         G(i,j,k)=1.0_PR/3.0_PR*(G0(i,j,k)+2.0_PR*(G1(i,j,k)+dt*dGdt(i,j,k)))
      enddo ; enddo ; enddo
  ...
  if(found) allocate(mat(1:M%Nl))
   ...
   do iph=1,M%Nl
      ...
      if(ph(iph)%typ.eq.1)then
        ...
        elseif(trim(adjustl(ph(iph)%nom)).eq.'eau2')then

            !ph(iph)=eau

            ph(iph)%p_sge=p_sge_eau2
            ph(iph)%g_sge=g_sge_eau2
            ph(iph)%mu=mu_eau2
            ph(iph)%sigy=sigy_eau2
            ph(iph)%rho0=rho0_eau2 
            ph(iph)%cv=cv_eau2
            ph(iph)%sig=sig_eau2
            ph(iph)%cv=cv_eau2
            ph(iph)%sig=sig_eau2
            ph(iph)%q_sge=q_eau2
            ph(iph)%qp_sge=qp_eau2
            ph(iph)%ener0=( 1.0e5_PR + ph(iph)%g_sge*ph(iph)%p_sge )/( (ph(iph)%g_sge - 1.0_PR)*ph(iph)%rho0 ) + ph(iph)%q_sge
